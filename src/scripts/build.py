...
def support_page_html(project, articles, copy, lang):
    ...
    if lang == 'de':
        intro = f'<section class="support-hero product-hero" aria-labelledby="support-product-title">{icon(project)}<p class="product-label">{escape(project["name"])}</p><label class="support-search"><span>Fragen durchsuchen</span><input type="search" data-support-search placeholder="Fragen durchsuchen" autocomplete="off"></label></section>'
        ...
        intro = f'<section class="support-hero product-hero" aria-labelledby="support-product-title">{icon(project)}<p class="product-label">{escape(project["name"])}</p><label class="support-search"><span>Fragen durchsuchen</span><input type="search" data-support-search placeholder="Fragen durchsuchen" autocomplete="off"></label></section>'
        contact = f'<p>Email me with the app name, app version, macOS version, and a short description of what happened. Steps to reproduce an issue are especially helpful.</p><p>Please remove personal information from screenshots and never include passwords, authentication secrets, or recovery codes.</p><div class="actions"><a class="button" href="mailto:{email}?subject=App%20Support">Contact</a></div>'
    return f'<div class="wrap"><header class="support-page-header">{intro}</header><div class="support-answers"><section class="answer-group is-open" id="{escape(project["slug"])}">{answers}</section></div><section class="contact-panel" id="contact">{contact}</section></div>'
